...
const GROUPS = [
  { key: "ocr" as const, title: "인식층 — 확률적", gate: "자기 신뢰도와 필드 단위 정확도를 함께 잰다" },
  { key: "ai" as const, title: "판단층 (AI) — 판단", gate: "κ_H를 먼저 재고 κ_H − κ_S로 잰다" },
    ...
    <Page
      ...
      lead={
        <>
          ...
          사람보다 낫다”가 아니라 <b>“사람의 판정을 표준화한다”</b>이며, 그래서 감리원 두 사람
          사이의 일치도 κ_H를 먼저 재고 시스템은 <b>κ_H − κ_S</b>로 잰다.
        </>
      }
    >
      <Note kind="warn">
      ...
      <div className="grid items-start gap-5 lg:grid-cols-3">
        ...
        >
          ...
          <p className="mt-2.5 rounded-md border border-[var(--sign)]/30 bg-[var(--sign-bg)] px-2.5 py-2 text-[12px] leading-relaxed text-[#1b3350]">
            어느 쪽이든 <b>두 사람의 일치도 κ_H를 먼저 재고, 2인 합의 라벨 대 시스템 κ_S를 그
            차이(κ_H − κ_S)로 잰다.</b> 칸 단위 일치율의 상한도 같은 표본의 2인 일치율이다. 사람
            둘도 갈리는 자리에서 시스템에 그 이상을 요구하지 않는다.
          </p>
        </Card>
        ...
        <Card title="반대 방향 표본을 일부러 섞는다">
          ...
          <p className="mt-2 rounded-md border border-[var(--rule)] bg-[#fbfaf7] px-2.5 py-2 text-[11.5px] leading-relaxed text-[var(--ink-2)]">
            ...
            오결합률은 정답 표본에서만 재고, <b>절단율은 감리원이 끊은 마디 비율이어서 오차가
            아니라 개입량이다.</b> 두 값이 벌어진 폭이 곧 조용히 남는 오결합의 크기다.
          </p>
        </Card>
      </div>
      ...
      {/* 기대효과 경계 */}
      <Card title="1차 산출은 달성률이 아니라 첫 기준선이다">
        <div className="grid gap-3 md:grid-cols-3">
          {[
            {
              t: "보수 (1단계)",
              items: [
                "확정 사건 N건(자동 제안 M건 중)과 정답 목록 대비 사건 재현율 — 1단계는 실측만 하고 합격선을 걸지 않는다",
                "대장 다섯 칸의 칸 단위 일치율 0.60 — 상한은 같은 표본의 2인 일치율이며 κ와 비교하지 않는다",
                "제출된 서식이 담은 건수와 대장이 되살린 건수의 차(정수)",
                "열나흘에 몰리던 별지 제50호 작성 부하가 사건 직후로 분산된다",
              ],
            },
            {
              t: "기준 (2단계)",
              items: [
                "칸 단위 일치율 0.75, 사건 재현율에 기준값 부여",
                "κ_H − κ_S ≤ 0.10 이면서 κ_S가 착수 문서의 절대 하한 이상",
                "불합격 이력을 찾는 시간 40% 단축 — 분모는 1단계 실측치이며 그 전에는 부르지 않는다",
                "사건 직후 확정 비율 (새 지표)",
              ],
            },
            {
              ...
              items: [
                ...
                "칸 정의를 채택한 협력사 수 — 판정 데이터와 원본 스캔은 공유하지 않는다",
