...
export const DISCLAIMER =
  "표시된 프로젝트·문서번호·장비 태그·수치는 모두 가상입니다. 실제 기업·현장·실적이 아닙니다.";
...
export const METRICS: Metric[] = [
  {
    layer: "ocr",
    name: "활자 검측조서 판독 가능 비율",
    value: "94.2%",
    num: 94.2,
    max: 100,
    gate: "합격선 90% 이상",
    pass: true,
    note: "표본 300장. 임계 미만 필드는 채우지 않고 「미확인」으로 남긴다",
  },
  {
    layer: "ocr",
    name: "「미확인」으로 남긴 필드 비율",
    value: "5.8%",
    num: 5.8,
    max: 100,
    gate: "채우지 않는 것이 정상 동작",
    pass: true,
    note: "손글씨 조치사항란에 몰려 있다. 손글씨는 2단계 범위다",
  },
  {
    layer: "ai",
    name: "유형 정규화 κ — 감리원 2인 κ 대비 도달률",
    value: "88%",
    num: 88,
    max: 100,
    gate: "2인 κ의 50% 미만이면 규칙 사전으로 환원",
    pass: true,
    note: "감리원 2인 κ = 0.71 이 상한 기준선. 시스템 κ = 0.62",
  },
  {
    layer: "ai",
    name: "사건 사슬 연결 정확도",
    ...
    gate: "참고 지표",
    pass: true,
    note: "문서번호가 끊긴 세 장을 위치·부위·공종·날짜로 잇는다",
  },
  {
    layer: "ai",
    name: "사슬 오결합률 (별도 지표)",
    ...
    num: 3.1,
    max: 10,
    gate: "합격선 5% 이하 — 초과 시 사슬 자동 제안 중지",
    pass: true,
    note: "잘못 이어붙인 사건은 대장에 조용히 남으므로 따로 잰다",
  },
  {
    layer: "det",
    name: "원본 해시 대조",
  ...
  {
    layer: "sign",
    name: "별지 제50호 칸 단위 일치율",
    value: "0.63",
    num: 63,
    max: 100,
    gate: "1단계 목표 0.6 (2인 κ 상한 이내)",
    pass: true,
    note: "정답셋 = 완료 현장이 실제 제출한 서식 또는 감리원 2인 수작성 대장",
  },
  {
    layer: "sign",
    ...
    num: 76,
    max: 100,
    gate: "HDX가 목표를 걸지 않은 새 지표",
    pass: true,
    note: "작성 시점이 준공 직전에서 사건 직후로 옮겨 갔는지를 잰다",
  },
];

export const STOP_RULES = [
  {
    when: "판독 가능 비율이 기준 미달",
    then: "스캔 경로를 중단하고, 2단계 초점을 전자 이슈 로그의 종결 사유 3분류로 먼저 돌린다",
  },
  {
    when: "사슬 오결합률이 합격선 초과",
    then: "사슬 자동 제안을 중지한다. 사람이 직접 잇는 방식으로 되돌린다",
  },
  {
    when: "유형 정규화 κ가 감리원 2인 κ의 절반 미만",
...
export const FORM50 = {
  ...
  formNo: "별지 제50호 서식",
  basis:
    "최종보고서 작성서식 — 대장에서 뽑아 낸 초안이다. 확정과 서명은 책임건설사업관리기술인이 한다",
...
export const ROADMAP = [
  {
    ...
    period: "0~6개월 (3개월 데모)",
    scope:
      "완료 3건, 기계 공종 활자 검측조서. 첫 4주는 개발이 아니라 판독률 실측 — 스캔 300장 표본",
    graft: "기존 문서 DB에 「판정 대장」 테이블 하나 추가. 유형 사전은 엑셀로 현업이 편집",
    out: "완료 1건의 판독률 수치 + 감리원 2인 κ + 별지 제50호 초안 1장 + 보유 현황 실사 3건",
  },
  {
    phase: "분기",
    period: "4주차",
    scope: "판독 가능 비율이 기준 미만인 경우",
    graft: "—",
    out: "2단계 초점을 손글씨가 아니라 전자 이슈 로그의 종결 사유 3분류로 먼저 돌린다",
  },
  {
    phase: "2단계",
    period: "6~18개월",
    scope: "진행 현장에 사건 직후 적재, 소방 감리로 확대",
    graft: "대장에서 별지 제50호 초안 생성",
    out: "세 지표 공표 — 판독률 · κ 도달률 · 오결합률",
  },
  {
    phase: "3단계",
    period: "18~36개월",
